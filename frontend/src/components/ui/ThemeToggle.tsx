...
export function ThemeToggle({ className = '' }: { className?: string }) {
  ...
  return (
    <div
      ...
      className={`inline-flex items-center gap-0.5 rounded-lg bg-surface-muted p-0.5 ${className}`}
    >
      {OPTIONS.map((option) => {
        ...
        return (
          <button
            ...
            className={`flex h-7 w-7 items-center justify-center rounded-md text-sm transition-colors ${
              selected ? 'bg-surface-elevated shadow-sm' : 'text-subtle hover:text-ink'
            }`}
          >
