...
export default function Error({
  ...
  return (
    ...
    >
      <p style={{ fontSize: 15, color: 'rgb(var(--color-ink, 32 32 36))' }}>{t('error.unexpected')}</p>

      {/* digest 는 서버 로그와 이 화면을 잇는 유일한 상관관계 키다. */}
      {error.digest && <code style={{ fontSize: 11, color: 'rgb(var(--color-muted, 107 107 115))' }}>{error.digest}</code>}
      ...
      <button
        ...
        style={{
          ...
          cursor: 'pointer',
          background: 'none',
          color: 'rgb(var(--color-ink, 32 32 36))',
          border: '1px solid rgb(var(--color-line, 222 222 226))',
          borderRadius: '8px',
        }}
      >
        {t('common.retry')}
