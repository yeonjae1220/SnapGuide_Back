 ...
 * 실제 클라이언트 IP를 추출한다.
 * 그렇지 않으면 nginx-ingress 뒤에서 모든 요청이 동일한 프록시 pod IP로 보여
 * IP 기반 rate limit이 클라이언트별이 아닌 전역 단일 버킷이 되거나,
 * 반대로 헤더 위조로 우회될 수 있다.
 */
@Component
public class ClientIpResolver {
    ...
    public ClientIpResolver(@Value("${snapguide.trusted-proxy-ips:127.0.0.1,::1}") String trustedProxyIpsConfig) {
    ...
    private IpAddressMatcher parseMatcher(String ip) {
        try {
            ...
            throw new IllegalStateException("snapguide.trusted-proxy-ips에 잘못된 IP/CIDR 항목이 있습니다: " + ip, e);
        }
    }
