...
public class CameraModelService {
    ...
    public CameraModel save(byte[] imageBytes) {
        // EXIF 메타데이터 추출
        MediaMetaData metaData = ExifExtractor.extract(imageBytes);
        // 카메라 모델 추출 && 저장
        CameraModel cameraModel = CameraModelExtractor.extract(imageBytes);
